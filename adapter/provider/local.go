...
func (p *LocalProvider) Start() error {
	if p.healchcheckHistory = service.FromContext[adapter.URLTestHistoryStorage](p.ctx); p.healchcheckHistory != nil {
	} else if clashServer := service.FromContext[adapter.ClashServer](p.ctx); clashServer != nil {
		p.healchcheckHistory = clashServer.HistoryStorage()
	} else {
...
func (p *LocalProvider) UpdateProvider(ctx context.Context, router adapter.Router) error {
	...
	if fileModeTime.Equal(p.lastUpdated) {
		return nil
	}
