...
func (g *myGroupAdapter) Now() string {
	return ""
}
...
func (g *myGroupAdapter) OutboundFilter(out adapter.Outbound) bool {
	return TestIncludes(out.Tag(), g.includes) && TestExcludes(out.Tag(), g.excludes) && TestTypes(out.Type(), g.types) && TestPorts(out.Port(), g.ports)
}
